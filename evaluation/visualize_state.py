...
def visualize_rotations(save_prefix, gt_rot, out_rot, inf_rot=None, save_folder=None):
    gt_euler = (
        np.unwrap(pp.SO3(gt_rot).euler(), axis=0, discont=np.pi / 2) * 180.0 / np.pi
    )
    outstate_euler = (
        np.unwrap(pp.SO3(out_rot).euler(), axis=0, discont=np.pi / 2) * 180.0 / np.pi
    )
    ...
    if inf_rot is not None:
        infstate_euler = (
            np.unwrap(pp.SO3(inf_rot).euler(), axis=0, discont=np.pi / 2)
            * 180.0
            / np.pi
        )
        for i in range(3):
...
def visualize_velocity(save_prefix, gtstate, outstate, refstate=None, save_folder=None):
    ...
    plt.tight_layout()
    if save_folder is not None:
        plt.savefig(os.path.join(save_folder, save_prefix + ".png"), dpi=300)
    plt.show()
    plt.close()

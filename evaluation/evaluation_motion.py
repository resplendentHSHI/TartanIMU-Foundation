...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--exp",
        type=str,
        default="experiments/euroc/motion_body",
        help="Path for AirIO netoutput",
    )
    parser.add_argument(
        "--seqlen", type=int, default="1000", help="the length of the segment"
    )
    parser.add_argument(
        "--dataconf",
        type=str,
    ...
    parser.add_argument(
        ...
        help="Directory where the results wiil be saved",
    )
    parser.add_argument(
        "--usegtrot",
        action="store_true",
        help="Use ground truth rotation for gravity compensation",
    )
    ...
    for data_conf in dataset_conf.data_list:
        ...
        for data_name in data_conf.data_drive:
            ...
            if args.exp is not None:
                ...
                indices = torch.cat(
                    [torch.where(gt_ts == item)[0] for item in vel_ts[:, 0]]
                ).to(torch.int32)

                if "coordinate" in dataset_conf.keys():
                    print("*************", dataset_conf["coordinate"], "*************")
                    if dataset_conf["coordinate"] == "body_coord":
                        rotation = motion_dataset.data[
                            "gt_orientation"
                        ]  # Set data['gt_orientation'] using AirIMU or gt-truth in the dataconf

                        vel_dist = (
                            rotation[indices, :] * inference_state["net_vel"]
                            - motion_dataset.data["velocity"][indices, :]
                        )
                        net_vel = interp_xyz(
                            gt_ts, vel_ts[:, 0], inference_state["net_vel"]
                        )
                        net_vel = rotation * net_vel

                    if dataset_conf["coordinate"] == "glob_coord":
                        vel_dist = (
                            inference_state["net_vel"]
                            - motion_dataset.data["velocity"][indices, :]
                        )
                        net_vel = interp_xyz(
                            gt_ts, vel_ts[:, 0], inference_state["net_vel"]
                ...
                integrator_vel = (
                    Velocity_Integrator(init["pos"]).to(args.device).double()
                )

                inf_outstate = integrate_pos(
                    integrator_vel, data_inte, init, motion_dataset, device=args.device
                )
